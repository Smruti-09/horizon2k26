...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function spinReels() {
    reels.forEach((reelId, i) => {
      ...
      // Target is always the first letter for simplicity — each reel's first letter matches
      // We spin through several full rotations then land on index 0
      const totalItems = reel.children.length;
      const fullSpins = 2 + i; // more spins for later reels
      const targetIndex = 0; // first div is the target letter
  ...
  function showResult() {
    if (resultEl) {
      resultEl.textContent = "SURYAKANTA!";
  ...
  // Auto-spin on load after a short delay
